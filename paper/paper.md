...
\caption{Modular architecture of TMMax, structured into three main components to facilitate efficient multilayer thin-film simulations. The Material Database (top left) comprises a curated set of \texttt{.npy} files containing refractive index and extinction coefficient data stored in the \texttt{nk_data} folder. The Optical Properties Calculation core (right) includes modular scripts for angle calculations, transfer matrix multiplications, Fresnel reflection or transmission, and material interpolation, among others, together performing all intermediate computations essential for the TMM. The Analysis section (bottom left) offers tools to evaluate the optical performance of thin-film stacks, including thickness-dependent transmittance, reflectance, absorbance, and resulting color. This modular structure enables high-performance, customizable thin-film modeling workflows.}
\end{figure*}

We designed TMMax to provide a comprehensive library that enables rapid simulations of multilayer optical thin films, while also integrating a publicly accessible material database and implementing extensive functionalities to facilitate the evaluation and characterization of thin film structures. As shown in Figure 2, we structured TMMax into three main components: the \texttt{nk_data} module, which serves as a curated material database; the optical properties computation core, responsible for all intermediate calculations required by the TMM framework; and the thin film analysis interface, which enables performance evaluation and characterization of multilayer structures. Since these parts rely on our JAX-based library, they adhere to functional programming principles. The design does not use classes or object-oriented structures; however, a user who prefers object-oriented design can easily encapsulate TMMax functions within their own classes. 
